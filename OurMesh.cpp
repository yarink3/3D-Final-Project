...
void OurMesh::init() {
    ...
    using namespace igl;
    FirstvectorToMoveBy=vectorToMoveBy;
    numcollapsed=0;
...
void OurMesh::changeDirections(int currentLevel){
//    vectorToMoveBy=vectorToMoveBy*(-1);
    if(vectorToMoveBy==Eigen::Vector3f(0.11,0,0)*currentLevel) {
        vectorToMoveBy = Eigen::Vector3f(0.11, 0.11, 0)*currentLevel;
        return;
    }
    if(vectorToMoveBy==Eigen::Vector3f(0.11,0.11,0)*currentLevel) {
        vectorToMoveBy = Eigen::Vector3f(-0.11, -0.11, -0.11)*currentLevel;
        return;
    }
    if(vectorToMoveBy==Eigen::Vector3f(-0.11, -0.11, -0.11)*currentLevel) {
        vectorToMoveBy = Eigen::Vector3f(0, -0.11, 0)*currentLevel;
        return;
    }
    if(vectorToMoveBy==Eigen::Vector3f(0, -0.11, 0)*currentLevel) {
        vectorToMoveBy = Eigen::Vector3f(-0.11, 0, 0)*currentLevel;
        return;
    }
    if(vectorToMoveBy == Eigen::Vector3f(-0.11, 0, 0)*currentLevel) {
        vectorToMoveBy = Eigen::Vector3f(0.11, 0, 0.11)*currentLevel;
        return;
    }
    if(vectorToMoveBy==Eigen::Vector3f(0.11, 0, 0.11)*currentLevel) {
        vectorToMoveBy = Eigen::Vector3f(-0.11, 0.11, 0)*currentLevel;
        return;
    }
    if(vectorToMoveBy==Eigen::Vector3f(-0.11, 0.11, 0)*currentLevel) {
...
//////////////////
    ...
    if(vectorToMoveBy==Eigen::Vector3f(-0.11,0,-0.11)*currentLevel) {
        vectorToMoveBy = Eigen::Vector3f(0,-0.11,0.11)*currentLevel;
        return;
    }

    if(vectorToMoveBy==Eigen::Vector3f(0,-0.11,0.11)*currentLevel) {
        vectorToMoveBy = Eigen::Vector3f(0.11,0.11,0.11)*currentLevel;
        return;
    }
    if(vectorToMoveBy==Eigen::Vector3f(0.11,0.11,0.11)*currentLevel) {
    ...
    if(vectorToMoveBy==Eigen::Vector3f(0,0,0.11)*currentLevel) {
        vectorToMoveBy = Eigen::Vector3f(-0.11, -0.11, 0)*currentLevel;
        return;
    }
    if(vectorToMoveBy==Eigen::Vector3f(-0.11, -0.11, 0)*currentLevel) {
